...
RSpec.describe GLRubocop::GLCops::TextAndContentVariableNaming do
  ...
  context 'with an instance variable lacking _text/_content suffix in a <p>' do
    ...
    it 'registers an offense' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `message` (line 1) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
  ...
  context 'with an instance variable lacking _text/_content suffix in a <span>' do
    ...
    it 'registers an offense' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `title` (line 1) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
  ...
  context 'with an improperly named body variable alongside attribute ERB' do
    ...
    it 'registers an offense for @thing' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `thing` (line 1) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
  ...
  context 'with a local variable lacking suffix' do
    ...
    it 'registers an offense' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `label` (line 1) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
  ...
  context 'with multiple bad variables in different elements' do
    ...
    it 'registers an offense for the first matching element' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `body` (line 1) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
  ...
  context 'with a variable on a later line' do
    ...
    it 'registers an offense' do
      expect_offense(<<~RUBY)
        render "component"
        ^^^^^^^^^^^^^^^^^^ GLCops/TextAndContentVariableNaming: `thing` (line 2) is rendered inside a text element. Rename it with a `_text` suffix (plain text) or `_content` suffix (HTML content).
      RUBY
    end
  end
end
